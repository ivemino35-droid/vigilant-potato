...
Ubuntu Pools adheres to a "Secure by Default" philosophy for the Phase 1 Pilot.

### Performance
- **Code Splitting:** Implemented via `React.lazy` to ensure minimal Time to Interactive (TTI).
- **Asset Optimization:** Images use explicit dimensions and lazy-loading to maintain a CLS of < 0.1.

### Security Compliance
- **Transport:** Mandatory HTTPS with HSTS preloading.
- **Headers:** Deny-based frame options to prevent clickjacking; strict CSP to mitigate XSS.
- **Privacy:** Robots.txt prevents crawling of sensitive admin paths during the pilot.
